...
[OUTPUT]: 对外提供模型填写人格小票 JSON 的唯一字段契约、票面微文案口径、行动签限制、长度限制、格式终检清单和输出规则
...
```text
证据 -> 动作化候选 -> app.v1 JSON -> app/index.html + app/app.js -> HTML/PNG
```
...
- `receiptId`: 票据 ID，例如 `PR_20260613_001_A7C3`；格式建议 `PR_YYYYMMDD_序号_随机码`，随机码用 4 位大写十六进制，避免多人分享时撞号。`序号` 必须和 `counter` 的数字一致，例如 `NO.001` 对应 `PR_20260613_001_A7C3`；同一任务多张小票时二者一起递增。
- `mbti`: MBTI 压缩标签，票面标签为 `MBTI`；不知道就写 `N/A`。部分确定时可写 `XNTJ`、`INXJ` 或 `XNTJ(部分确定)`，不要为了凑四字母写成诊断。它是大众索引，不是整张票的灵魂。
...
- `total`: 中文总计，最多 8 个汉字；它是整张票的金额总计，是用户运行模式的最终价值缩影。它不是内容摘要，而是一句话的人格定性。禁止标点、空格和英文。优先写成对 `mbti` 的精修或反叛，例如 `精密但短路`、`柔软但有刺`、`先结构后行动`、`深夜探照灯`。
...
- `verdict`: 英文判词，硬上限 32 个字符，目标区间 20-25 个可见字符；它来自决策模式与压力模式的张力，不写随机格言。避免整句解释和过多 `W`、`M` 这类宽字符，防止热敏纸窄排折行。
...
mbti -> total
  mbti 给大众索引，total 给精修判断。total 必须比 mbti 更具体。

decision x stress -> verdict
  判词写决策方式和压力反应之间的张力，例如 QUIET MODE. HARD EDGE.

type + 五维极端值 -> rarity
  越极端越稀有；低证据不装稀有，用云团和保守百分比。

type + total + stress -> typeGlyph
  无图印章要像这个 type 的低分辨率影子，不是装饰贴纸。
```
...
- `total` 是否是整张票的最终缩影，而不是普通摘要？
- `type` 不是诊断、身份或优劣评价。
- `verdict` 是否来自 decision 和 stress 的张力，而不是随机漂亮话？
