...
<directory>
references/ - 逆向设计规则资源，负责 JSON 预处理、低置信槽位探针、唯一输出契约、类型素材查表和安装查漏
...
references/: 逆向设计规则层，承载 JSON 预处理手册、低置信槽位探针、唯一输出契约、类型素材查表和安装查漏协议。
...
`SKILL.md` 只放流程和路由。`references/` 只放能让 app.v1 JSON 槽位更确定的规则：预处理手册、探针、唯一契约、类型素材表和安装查漏。`assets/` 放 app.v1 样例和契约模板。`app/` 是唯一票面 HTML 模板与浏览器导出真相源。`scripts/` 只做非交互包装，把 JSON 注入 app，保存单张 HTML fixture 并拿 PNG。`test/` 放 app 链路验收样例和导出物。`agents/` 只放产品界面元数据，不参与人格判断。
